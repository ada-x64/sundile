	...
	namespace /* Event catchers */ {
		void eventSystemInit(const InitEvent<SmartEVW>& ev) {
			for (auto window : windows) {
				InitEvent<SmartWindow> ev{ window };
				ev.evw->dispatcher.trigger<InitEvent<SmartWindow>>(wev);
			}
		}

		void catchTerminateEvent(const TerminateEvent<SmartWindow>& wev) {
			terminate(wev.member);
		}
		...
		void postStep(const PostStepEvent& ev) {
			for (auto window : windows) {
				window->evw->dispatcher.update<TerminateEvent<SmartWindow>>();
			}
		}

		void setEventCallbacks(SmartEVW evw) {
			evw->dispatcher.sink<TerminateEvent<SmartWindow>>().connect<catchTerminateEvent>();
